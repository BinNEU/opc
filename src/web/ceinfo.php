...
$ok=false;
$id=intval($_GET['sid']);//为什么id都需要用intval函数？？？
$sql="SELECT * FROM `solution` WHERE `solution_id`=?";
$result=pdo_query($sql,$id);
$row=$result[0];
if ($row && $row['user_id']==$_SESSION[$OJ_NAME.'_'.'user_id']) $ok=true;//验证身份
...
if ($ok==true){
	...
	$sql="SELECT `error` FROM `compileinfo` WHERE `solution_id`=?";//WEB轮寻出编译信息
	$result=pdo_query($sql,$id);
	 $row=$result[0];
	if($row&&is_valid($row['error'])){
	$view_reinfo= htmlentities(str_replace("\n\r","\n",$row['error']),ENT_QUOTES,"UTF-8");}//$row['error']即为编译报错信息 查找到编译的信息并存在前面数组中
	else{
		$view_reinfo="成功！";
	}
