...
if((isset($_GET['week'])&&trim($_GET['week'])!="")){
	...
	if (!$Newresult){
     ...
     $sql="SELECT contest.contest_id, contest.title,contest.start_time,contest.end_time,contest.description,contest.private FROM contest WHERE contest.defunct = 'N' and end_time>now()";
		$resultproblem=mysql_query_cache($sql);	
	if(!$resultproblem){
