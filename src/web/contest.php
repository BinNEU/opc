	...
	if (isset($_GET['cid'])){
		...
		//	print $cid;
			...
			if ($rows_cnt==0){
				...
				if($exam==1&&!(isset($_GET['eid'])))
				{
					$view_errors=  "<h2>请从考试入口进入</h2>";
		...
		order by pnum
			...
			$cnt=0;
			 foreach($result as $row){
				...
				if(isset($eid))
				$view_problemset[$cnt][2]= "<a href='contestproblem.php?cid=$cid&pid=$cnt&eid=$eid'>".$resulttitle."</a>";
				else
				$view_problemset[$cnt][2]= "<a href='contestproblem.php?cid=$cid&pid=$cnt'>".$resulttitle."</a>";
...
$pend=$page_cnt;
$rows=pdo_query("select count(1) from contest where defunct='N' and exam=0");
...
//  echo "$mycontests";
  ...
  $sql="SELECT * FROM `contest` WHERE `defunct`='N' and exam=0 ORDER BY `contest_id` DESC limit 1000";
  if($keyword){
	$sql="select *  from contest left join (select * from privilege where rightstr like 'm%') p on concat('m',contest_id)=rightstr where contest.defunct='N' and contest.title like ? and contest.exam=0 $wheremy  order by contest_id desc ";
	
	$sql.=" limit ".strval($pstart).",".strval($pend); 
	$result=pdo_query($sql,$keyword);
  }else{
	$sql="select *  from contest left join (select * from privilege where rightstr like 'm%') p on concat('m',contest_id)=rightstr where contest.defunct='N' and contest.exam=0 $wheremy  order by contest_id desc ";
	$sql.=" limit ".strval($pstart).",".strval($pend); 
