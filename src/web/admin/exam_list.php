...
$sql="SELECT max(`contest_id`) as upid, min(`contest_id`) as btid, count(1) as cnt FROM `contest` where exam=1";
$page_cnt=1000;
...
if(isset($_GET['keyword'])){
	...
	$keyword="%$keyword%";
	 $sql="select `contest_id`,`title`,`start_time`,`end_time`,`private`,`defunct` FROM `contest` where title like ? and exam=1";
	 $result=pdo_query($sql,$keyword);
}else{
     $sql="select `contest_id`,`title`,`start_time`,`end_time`,`private`,`defunct` FROM `contest` where contest_id>=? and contest_id <=? and exam=1 order by `contest_id` desc";
	 $result=pdo_query($sql,$pstart,$pend);
}
?>
<div style="float:left;">
<form action=exam_list.php class="pagination" ><input name=keyword><input type=submit value="<?php echo $MSG_SEARCH?>" ></form>
</div><div style="display:inline;">
<nav class="center"><ul class="pagination">
<li class="page-item"><a href="exam_list.php?page=1">&lt;&lt;</a></li>
<?php
...
for ($i=$start;$i<=$end;$i++){
 echo "<li class='".($page==$i?"active ":"")."page-item'>
            <a title='go to page' href='exam_list.php?page=".$i.(isset($_GET['my'])?"&my":"")."'>".$i."</a></li>";
}
?>
<li class="page-item"><a href="exam_list.php?page=<?php echo $cnt?>">&gt;&gt;</a></li>
</ul>
</nav>
</div>
<?php
echo "<div class='layui-form' id='table-list'><table class='table table-striped' width=90% border=1>";
echo "<form method=post action=exam_add.php>";
echo "<input type=checkbox lay-skin='primary' onchange='$(\"input[type=checkbox]\").prop(\"checked\", this.checked)'>";
echo "<input class='layui-btn layui-btn-normal'  type=submit name='contest2exam' value='CheckToNewExam'>";
echo "<thead>";
echo "<tr><td><td>ContestID<td>Title<td>StartTime<td>EndTime<td>Private<td>Status<td>Edit<td>Copy<td>Export<td>Logs";
echo "</tr></thead>";
foreach($result as $row){
        echo "<tr>";
		echo "<td><input type=checkbox lay-skin='primary' name='pid[]' value='".$row['contest_id']."'>";
