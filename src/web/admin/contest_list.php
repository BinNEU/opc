...
$sql="SELECT max(`contest_id`) as upid, min(`contest_id`) as btid, count(1) as cnt FROM `contest` where exam=0";
$page_cnt=10000;
...
if(isset($_GET['keyword'])){
	...
	$keyword="%$keyword%";
	 $sql="select `contest_id`,`title`,`start_time`,`end_time`,`private`,`defunct` FROM `contest` where title like ? and exam=0";
	 $result=pdo_query($sql,$keyword);
}else{
     $sql="select `contest_id`,`title`,`start_time`,`end_time`,`private`,`defunct` FROM `contest` where contest_id>=? and contest_id <=? and exam=0 order by `contest_id` desc";
