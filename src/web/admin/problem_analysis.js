...
for(let i=0; i<clcs.length; i++){
    ...
    if(regrst!=null){
        var unit = {name: regrst[1], students: regrst[2].split(/\s*,\s*/g)};
        classes.push(unit);
    }
}
...
//总览
{
    ...
    //时间占用分布表（按人头）
    {
...

        {
            ...
            for(var i=0; i<selectedPdo.length; i++){
                var value = [(selectedPdo.length-i-1)/selectedPdo.length, selectedPdo[i].time, selectedPdo[i]];
        ...
        //内存占用分布表（按人头）
        {
...

            {
                ...
                for(var i=0; i<selectedPdo.length; i++){
                    var value = [(selectedPdo.length-i-1)/selectedPdo.length, selectedPdo[i].memory, selectedPdo[i]];
