...
<div class="container layout layout-margin-top">
    ...
    <div class="row">
		...
		<div class="content">
		<center>
	<h2><?php echo $name?>的答题总览</h2></center>
<caption>
...
$sql="SELECT UNIX_TIMESTAMP(date(in_date))*1000 md,count(1) as c,count(if(result=4,true,null)) as ac FROM `solution` where  `user_id`=?  group by md order by md asc  ";
	$result2=pdo_query($sql,$prouser);//mysql_escape_string($sql));
	$row=$result2[0];
			?>
...
$sql="select count(*) al,count(if(result=4,true,null)) ac ,point from (SELECT solution.problem_id,point,result FROM `solution`,problem WHERE `user_id`=? and solution.problem_id=problem.problem_id) a group by point";
	$result3=pdo_query($sql,$prouser);//mysql_escape_string($sql));
			?>
                   
          	  <div>
			  		<h4><strong>AC题目</strong></h4><br>
...
if ($result=pdo_query($sql,$prouser)){ 
...
<div class="sidebox">
	<div class="sidebox-header">
		<h4 class="sidebox-title">Download All AC Source</h4>
	</div>
          <div class="row userinfo-data">
  ...
  <a href="<?php echo $path_fix?>export_ac_code.php" type="button" class="btn btn-success col-md-4 col-xs-6 col-md-offset-1 register-btn" style='color: #fff;background-color: #3baeda;border: 1px solid #3baeda;border-radius: 4px;'>点击下载</a>																		
            </div>
...
<div class="sidebox">
	...
	<div class="sidebox-header">
		<h4 class="sidebox-title">答题数据</h4>
	</div>
	<div class="sidebox-body course-content side-list-body">
        <a><?php echo $MSG_Number?>：<?php echo $Rank?></a>
        <a href='status.php?user_id=<?php echo $user?>&jresult=4'><?php echo $MSG_SOVLED?>：<?php echo $AC?></a>
        <a href='status.php?user_id=<?php echo $user?>'><?php echo $MSG_SUBMIT?>：<?php echo $Submit?></a>
		<?php
foreach($view_userstat as $row){
//i++;
echo "<a href=status.php?user_id=$user&jresult=".$row[0]." >".$jresult[$row[0]]."：".$row[1]."</a>";
}
//}
?>
	</div>
