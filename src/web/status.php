...
$row=$result[0];
$view_end_times=$row['end_time'];
$title=$row['title'];
$file=$row['file'];
$timestamp=strtotime($view_end_times);
...
if (isset($_GET['solution_id'])&&$_GET['solution_id']!=""){
	...
	if(isset($_GET['sid'])){
		$solution_id=htmlentities($_GET['solution_id'],ENT_QUOTES,'UTF-8');
		$num=strpos($PID,$solution_id);
		$solution_id=$PID[$num];
		$sql=$sql."AND `num`='".$num."' ";
        $str2=$str2."&solution_id=".trim($solution_id);
        
	}else{
        $solution_id=strval(intval($_GET['solution_id']));
        if ($solution_id!='0'){
                $sql=$sql."AND `solution_id`='".$solution_id."' ";
                $str2=$str2."&solution_id=".trim($solution_id);
        }
        else $solution_id="";
	}
...
for ($i=0;$i<$rows_cnt;$i++){
	   ...
	   $view_status[$i][0]=$row['solution_id'];
        ...
        if ($row['contest_id']>0) {
			...
			if(isset($eid)){
                	$view_status[$i][1]= "<a>".$row['user_id']."</a>";
			}else
				$view_status[$i][1]= "<a href='contestrank.php?cid=".$row['contest_id']."&user_id=".$row['user_id']."#".$row['user_id']."'>".$row['user_id']."</a>";
	   }else{
                if (isset($_SESSION[$OJ_NAME.'_'.'administrator']))
			$view_status[$i][1]= "<a href='analysis.php?user=".$row['user_id']."' title='".$row['ip']."'>".$row['user_id']."</a>";
		else
                	$view_status[$i][1]= "<a href='analysis.php?user=".$row['user_id']."'>".$row['user_id']."</a>";
        }
       if ($row['contest_id']>0) {
		     if(isset($eid))
				 $view_status[$i][2]= "<div class=center><a href='contestproblem.php?cid=".$row['contest_id']."&pid=".$row['num']."&eid=".$eid."'>";
				else
                $view_status[$i][2]= "<div class=center><a href='contestproblem.php?cid=".$row['contest_id']."&pid=".$row['num']."'>";
                if(isset($cid)){
	...
	$view_status[$i][3]="<span class='hidden' style='display:none' result='".$row['result']."' ></span>";
        if (intval($row['result'])==11 && ((isset($_SESSION[$OJ_NAME.'_'.'user_id'])&&$row['user_id']==$_SESSION[$OJ_NAME.'_'.'user_id']) || isset($_SESSION[$OJ_NAME.'_'.'source_browser'])|| isset($_SESSION[$OJ_NAME.'_'.'administrator']))){
        	...
        	if ($row['result']!=4&&isset($row['pass_rate'])&&$row['pass_rate']>0&&$row['pass_rate']<.98)
                                $view_status[$i][3].= ($row['pass_rate']*100)."分</a>";
		else
	      			$view_status[$i][3].="</a>";
        	...
        	if ($row['result']!=4&&isset($row['pass_rate'])&&$row['pass_rate']>0&&$row['pass_rate']<.98)
              ...
              if(!$lock||$lock_time>$row['in_date']||$row['user_id']==$_SESSION[$OJ_NAME.'_'.'user_id']){
        		...
        		if ($row['result']!=4&&isset($row['pass_rate'])&&$row['pass_rate']>0&&$row['pass_rate']<.98)
                                $view_status[$i][3].= ($row['pass_rate']*100)."分</span>";
			else
				$view_status[$i][3].="</span>";
                        if( isset($_SESSION[$OJ_NAME.'_'.'source_browser'])||isset($_SESSION[$OJ_NAME.'_'.'administrator'])||isset($_SESSION[$OJ_NAME.'_'.'problem_editor'])){
        ...
        if ($flag){
                ...
                if (!(isset($_SESSION[$OJ_NAME.'_'.'administrator']) || isset($_SESSION[$OJ_NAME.'_'.'problem_editor'])||isset($_SESSION[$OJ_NAME.'_'.'user_id'])&&strtolower($row['user_id'])==strtolower($_SESSION[$OJ_NAME.'_'.'user_id']) || isset($_SESSION[$OJ_NAME.'_'.'source_browser']))){
			...
			if($row["problem_id"]>0){
                        	if ($row['contest_id']>0) {
								if(isset($eid))
									$view_status[$i][6].= "/<a target=_self href=\"submitpage.php?cid=".$row['contest_id']."&pid=".$row['num']."&sid=".$row['solution_id']."&eid=".$eid."\">Edit</a>";
									else
                                	$view_status[$i][6].= "/<a target=_self href=\"submitpage.php?cid=".$row['contest_id']."&pid=".$row['num']."&sid=".$row['solution_id']."\">Edit</a>";
                        	}else{
                                	$view_status[$i][6].= "/<a target=_self href=\"problem.php?id=".$row['problem_id']."&sid=".$row['solution_id']."\">Edit</a>";
                        	}
			}
                }
