...
for ($i=0;$i<$rows_cnt;$i++){
       ...
       if ($row['contest_id']>0) {
                $view_status[$i][2]= "<div class=center><a href='appproblem.php?cid=".$row['contest_id']."&pid=".$row['num']."'>";
                if(isset($cid)){
                ...
                $view_status[$i][2]= "<div class=center><a href='appproblem.php?id=".$row['problem_id']."'>".$row['problem_id']."</a></div>";
        }
	switch($row['result']){
		case 4:
	...
	$view_status[$i][3]="<span class='hidden' style='display:none' result='".$row['result']."' ></span>";
        if (intval($row['result'])==11 && ((isset($_SESSION[$OJ_NAME.'_'.'user_id'])&&$row['user_id']==$_SESSION[$OJ_NAME.'_'.'user_id']) || isset($_SESSION[$OJ_NAME.'_'.'source_browser']))){
        	...
        	if ($row['result']!=4&&isset($row['pass_rate'])&&$row['pass_rate']>0&&$row['pass_rate']<.98)
              ...
              if(!$lock||$lock_time>$row['in_date']||$row['user_id']==$_SESSION[$OJ_NAME.'_'.'user_id']){
                if(false&&$row['sim']>80&&$row['sim_s_id']!=$row['s_id']) {
				...
				$view_status[$i][3].="</span>";
                        if( isset($_SESSION[$OJ_NAME.'_'.'source_browser'])||isset($_SESSION[$OJ_NAME.'_'.'administrator'])){
                                        $view_status[$i][3].= "<a  class='label label-info'  target=original>".$row['sim_s_id']."(".$row['sim']."%)</a>";
                        }else{
        ...
        if ($flag){
                ...
                if (!(isset($_SESSION[$OJ_NAME.'_'.'administrator']) || isset($_SESSION[$OJ_NAME.'_'.'problem_editor'])||isset($_SESSION[$OJ_NAME.'_'.'user_id'])&&strtolower($row['user_id'])==strtolower($_SESSION[$OJ_NAME.'_'.'user_id']) || isset($_SESSION[$OJ_NAME.'_'.'source_browser']))){
                        ...
                        $view_status[$i][6]= "<a target=_blank >".$language_name[$row['language']]."</a>";
			if($row["problem_id"]>0){
                        	if ($row['contest_id']>0) {
                                	$view_status[$i][6].= "/<a target=_self >Edit</a>";
                        	}else{
                                	$view_status[$i][6].= "/<a target=_self>Edit</a>";
                        	}
			}
...
if (isset($_GET['cid']))
	...
	require("template/".$OJ_TEMPLATE."/appstatus.php");
/////////////////////////Common foot
