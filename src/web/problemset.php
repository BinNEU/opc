...
if (isset($_SESSION[$OJ_NAME.'_'.'administrator'])){
	...
	$sql="SELECT `problem_id`,`title`,`source`,`submit`,`accepted`,`school`,`point`,`leitai` FROM `problem` ".
	"WHERE `defunct`='N' and  $filter_sql and `leitai` IS NULL AND `problem_id` NOT IN(
		SELECT  `problem_id` 
		FROM contest c
...
foreach ($result as $row){
	...
	if(is_null($row['leitai']))
	$view_problemset[$i][3]="<div class='center'><nobr>".$row['source']."</nobr></div >";
    else
	$view_problemset[$i][3]="<div class='center'><nobr>擂台赛</nobr></div >";
	...
	$view_problemset[$i][6]="<div class='center'>".$row['point']."</a></div>";
	$view_problemset[$i][7]="<div class='center'>".$row['school']."</a></div>";
