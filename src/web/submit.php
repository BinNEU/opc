...
$eid=intval($_POST['eid']);
        $sql="SELECT count(1) FROM `solution` WHERE result<4";//result<4是什么意思？
        $result=mysql_query_cache($sql);
         $row=$result[0];
        if($row[0]>50) $OJ_VCODE=true;
        
if($OJ_VCODE)$vcode=$_POST["vcode"];//这里的句法？？？？？？？？？？？？
$err_str="";
$err_cnt=0;
...
$res=mysql_query_cache($sql);//没有查询到
...
$sqls="select * from problem where problem_id=?";
$hanshuresult=pdo_query($sqls,$id);
$rows=$hanshuresult[0];
$zhuhanshu=$rows['zhuhanshu'];
$hanshu=$rows['code'];
$buquan=$rows['code2'];
if(strlen($buquan)&&strlen($hanshu)){
	$hanshu=$hanshu."\n";
	$source=$hanshu.$source;
	$source=$source."\n";
	$source=$source.$buquan;
}else if(strlen($zhuhanshu)){
	$zhuhanshu=$zhuhanshu."\n";
	$source=$zhuhanshu.$source;
}
//end of append 
if($language==6)
   $source="# coding=utf-8\n".$source;
...
$len=strlen($source);//source即为客户端输入的代码
//echo $source;
...
if((~$OJ_LANGMASK)&(1<<$language)){//插入轮寻表？？？
	...
	//using redis task queue   ??????
	 ...
	 $statusURI=strstr($_SERVER['REQUEST_URI'],"submit",true)."status.php";//跳转到 status.php
	 if (isset($cid)) 
	    $statusURI.="?cid=$cid";
   ...
   // echo $statusURI."<br>";
    ...
    if($OJ_MEMCACHE){
	 ...
	 if (isset($app))
    //echo $file;
	$statusURI="appstatus.php?user_id=".$_SESSION[$OJ_NAME.'_'.'user_id'];
	else
